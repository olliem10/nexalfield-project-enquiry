...
--
-- Nothing applies this automatically on deploy — it is run by hand with
-- `npm run db:migrate` — but that can still be run more than once against a
-- database that already has this schema, so every statement below is safe to
-- re-run: nothing is dropped, no data is touched, and a second run simply
-- does nothing. Later migrations should follow the same rule.
