 ...
 *
 * Generic in the context type so each function can accept Netlify's full
 * `Context` while the helpers here only ask for the parts they use.
 */
...
/**
 * Best-effort caller identity for rate limiting. Netlify populates
 * `context.ip`; the forwarded header is a fallback for local development.
 */
export function clientIp(request: Request, context: NetlifyContextLike): string {
...
/** Removes counters whose window closed long ago. Called from the hourly job. */
...
export function requireEnv(name: string): string {
  ...
  if (!value) {
    throw new HttpError(
      503,
      'not_configured',
      `This site is missing the ${name} environment variable. Set it in Netlify under Site configuration → Environment variables.`,
    )
  }
  return value
}
