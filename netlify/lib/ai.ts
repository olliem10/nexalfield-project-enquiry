 ...
 *
 * Netlify's AI Gateway injects the credentials, so no key is set by hand. This
 * module never throws: every outcome is a value, because a submission must not
 * depend on a model being available. A failed summary is recorded, shown in the
 * dashboard, retried hourly, and can be regenerated on demand.
 */
...
export async function generateProjectSummary(
  ...
  const client = new Anthropic({
    ...
    // Netlify's AI Gateway sets this; the SDK's own default is used otherwise.
